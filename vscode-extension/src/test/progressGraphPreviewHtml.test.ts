...
test('buildProgressGraphPreviewHtml renders scheduler operator candidates and explicit action buttons', () => {
  ...
  assert.match(html, /querySelectorAll\('\[data-pg-workflow-evidence-prefill\]'\)/);
  assert.match(html, /target\.dataset\.pgWorkflowEvidenceId \|\| ''/);
  assert.match(html, /target\.dataset\.pgWorkflowEvidencePath \|\| ''/);
  assert.match(html, /pgHostSandboxWorkflowAllocationEvidenceId/);
  assert.match(html, /pgHostSandboxWorkflowAllocationEvidencePath/);
  const workflowPrefillHandler = html.match(/for \(const button of document\.querySelectorAll\('\[data-pg-workflow-evidence-prefill\]'\)\) \{[\s\S]*?\n  \}\n  const readTextInput/)?.[0] ?? '';
  assert.ok(workflowPrefillHandler, 'workflow prefill handler should be rendered');
  assert.doesNotMatch(workflowPrefillHandler, /postMessage/);
  assert.doesNotMatch(workflowPrefillHandler, /pgHostCleanupEvidencePath/);
  assert.doesNotMatch(workflowPrefillHandler, /pgHostSandboxWorkflowCleanup/);
...
test('buildProgressGraphPreviewHtml renders selectable cleanup receipt evidence candidates', () => {
  ...
  assert.match(html, /data-pg-workflow-evidence-prefill="true"/);
  assert.match(html, /Use for workflow/);
  assert.match(html, /data-pg-cleanup-evidence-path="\.codex\/scheduler\/evidence\/allocation-receipts\.json"/);
  assert.match(html, /data-pg-workflow-evidence-id="allocation-receipts"/);
  assert.match(html, /data-pg-workflow-evidence-path="\.codex\/scheduler\/evidence\/allocation-receipts\.json"/);
  assert.match(html, /data-pg-cleanup-evidence-path="E:\/workspace\/example\/\.codex\/scheduler\/evidence\/source-allocation\.json"/);
  assert.match(html, /data-pg-workflow-evidence-id="source-allocation"/);
  assert.match(html, /data-pg-workflow-evidence-path="E:\/workspace\/example\/\.codex\/scheduler\/evidence\/source-allocation\.json"/);
