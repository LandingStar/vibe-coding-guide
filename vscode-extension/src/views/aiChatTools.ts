...
const AI_CHAT_TOOL_DEFINITIONS: readonly AiChatToolDefinition[] = [
    ...
    },
    {
        ...
        description: 'Agent-owned Local Work Trajectory mutation. Use start at task start, append for planned/observed events, advance when active event is complete, update to refine current event, block/wait for impediments, resume to continue, and close when the single-line task is done. When starting, pass sourceGraphId and sourceNodeId if the owning global progress graph node is known so the trajectory is visible from birth. Use addLane early when one distinct work context exists. Use addLanes when one decision expands several distinct lanes at once, especially when server/client/protocol/testing or similarly separable streams can progress with their own context and later rejoin. Use addCompound for a planned compound phase, packRange for a same-lane interval, and packSubgraph for multiple lane-local continuous ranges. Use merge to add an explicit target-lane merge event and a merges_into relation from a source lane event. Use relate to record explicit dependency/wait/unblock/handoff/sync/approval metadata between existing events; optional endpoint fields can point inside compound child trajectories. Use setAnchor to move or clear the current trajectory anchor later. It does not schedule work or resolve conflicts. After validation or delivery completes, keep advancing until completed milestones are not left pending or in_progress. This is not a user-facing control.',
        args: '{ action: "start"|"append"|"advance"|"update"|"block"|"wait"|"resume"|"close"|"addLane"|"addLanes"|"addCompound"|"packRange"|"packSubgraph"|"appendChild"|"advanceChild"|"closeChild"|"merge"|"relate"|"setAnchor", laneLabel?: string, firstEventTitle?: string, title?: string, eventKind?: string, relationKind?: "depends_on"|"waits_for"|"unblocks"|"hands_off"|"syncs_from"|"merges_into"|"proposes_new_line"|"approves_new_line", summary?: string, reason?: string, guideContext?: string, currentEventId?: string, laneId?: string, lanes?: Array<{ laneLabel?: string, lane_label?: string, firstEventTitle?: string, first_event_title?: string, eventKind?: string, event_kind?: string, summary?: string, laneId?: string, lane_id?: string }>, sourceEventId?: string, sourceLaneId?: string, targetLaneId?: string, targetEventId?: string, sourceGraphId?: string, sourceNodeId?: string, packRanges?: Array<{ laneId?: string, lane_id?: string, rangeStartEventId?: string, range_start_event_id?: string, rangeEndEventId?: string, range_end_event_id?: string }>, anchorLaneId?: string, sourceEndpointTrajectoryId?: string, sourceEndpointEventId?: string, sourceEndpointParentEventId?: string, sourceEndpointCompoundPath?: string, targetEndpointTrajectoryId?: string, targetEndpointEventId?: string, targetEndpointParentEventId?: string, targetEndpointCompoundPath?: string }',
    },
] as const;
...
export class AiChatToolExecutor {
    ...
    private async _localTrajectory(args: Record<string, unknown>): Promise<AiChatToolResult> {
        ...
        if (!['start', 'append', 'advance', 'update', 'block', 'wait', 'resume', 'close', 'addLane', 'addLanes', 'addCompound', 'packRange', 'packSubgraph', 'appendChild', 'advanceChild', 'closeChild', 'merge', 'relate', 'setAnchor'].includes(action)) {
            return this._error('localTrajectory', 'localTrajectory action must be start, append, advance, update, block, wait, resume, close, addLane, addLanes, addCompound, packRange, packSubgraph, appendChild, advanceChild, closeChild, merge, relate, or setAnchor.');
        }
        ...
        const payload = {
            ...
            lanes: Array.isArray(args.lanes) ? args.lanes : [],
            ...
            firstChildEventTitle: this._readOptionalString(args.firstChildEventTitle) ?? '',
            childLaneLabel: this._readOptionalString(args.childLaneLabel) ?? '',
            rangeStartEventId: this._readOptionalString(args.rangeStartEventId) ?? '',
            rangeEndEventId: this._readOptionalString(args.rangeEndEventId) ?? '',
            packRanges: Array.isArray(args.packRanges) ? args.packRanges : [],
            anchorLaneId: this._readOptionalString(args.anchorLaneId) ?? '',
            parentEventId: this._readOptionalString(args.parentEventId) ?? '',
            childTrajectoryId: this._readOptionalString(args.childTrajectoryId) ?? '',
            sourceEndpointTrajectoryId: this._readOptionalString(args.sourceEndpointTrajectoryId) ?? '',
            sourceEndpointEventId: this._readOptionalString(args.sourceEndpointEventId) ?? '',
            sourceEndpointParentEventId: this._readOptionalString(args.sourceEndpointParentEventId) ?? '',
            sourceEndpointCompoundPath: this._readOptionalString(args.sourceEndpointCompoundPath) ?? '',
            targetEndpointTrajectoryId: this._readOptionalString(args.targetEndpointTrajectoryId) ?? '',
            targetEndpointEventId: this._readOptionalString(args.targetEndpointEventId) ?? '',
            targetEndpointParentEventId: this._readOptionalString(args.targetEndpointParentEventId) ?? '',
            targetEndpointCompoundPath: this._readOptionalString(args.targetEndpointCompoundPath) ?? '',
            sourceGraphId: this._readOptionalString(args.sourceGraphId) ?? '',
            sourceNodeId: this._readOptionalString(args.sourceNodeId) ?? '',
        };
        const script = [
            ...
            '    pass',
            'from tools.progress_graph import add_local_work_compound, add_local_work_lane, add_local_work_lanes, add_local_work_relation, advance_local_work_child_event, advance_single_line_event, append_local_work_child_event, append_single_line_event, block_single_line_event, close_local_work_child_trajectory, close_single_line_trajectory, load_local_work_trajectory, merge_local_work_lane, pack_local_work_range, pack_local_work_subgraph, resume_single_line_event, set_local_work_trajectory_anchor, start_single_line_trajectory, update_single_line_event',
            '',
            'root = Path.cwd()',
            ...
            '        source_graph_id=action.get("sourceGraphId") or "",',
            '        source_node_id=action.get("sourceNodeId") or "",',
            '    )',
            'elif kind == "append":',
            ...
            '    if not lanes:',
            '        raise ValueError("localTrajectory addLanes requires lanes")',
            '    path = add_local_work_lanes(',
            '        root,',
            '        lanes=lanes,',
            '        source_event_id=action.get("sourceEventId") or action.get("currentEventId") or None,',
            '    )',
            'elif kind == "addCompound":',
            '    path = add_local_work_compound(',
            '        root,',
            '        title=action.get("title") or "compound work",',
            '        summary=action.get("summary") or "",',
            '        lane_id=action.get("laneId") or "",',
            '        first_child_event_title=action.get("firstChildEventTitle") or action.get("firstEventTitle") or "",',
            '        first_child_event_kind=action.get("eventKind") or "task",',
            '        first_child_event_summary=action.get("summary") if (action.get("firstChildEventTitle") or action.get("firstEventTitle")) else "",',
            '        child_lane_label=action.get("childLaneLabel") or action.get("laneLabel") or action.get("title") or "compound work",',
            '    )',
            'elif kind == "packRange":',
            '    path = pack_local_work_range(',
            '        root,',
            '        title=action.get("title") or "packed range",',
            '        range_start_event_id=action.get("rangeStartEventId") or action.get("sourceEventId") or "",',
            '        range_end_event_id=action.get("rangeEndEventId") or action.get("targetEventId") or "",',
            '        summary=action.get("summary") or "",',
            '        child_lane_label=action.get("childLaneLabel") or action.get("laneLabel") or action.get("title") or "packed range",',
            '    )',
            'elif kind == "packSubgraph":',
            '    path = pack_local_work_subgraph(',
            '        root,',
            '        title=action.get("title") or "packed subgraph",',
            '        ranges=action.get("packRanges") or [],',
            '        anchor_lane_id=action.get("anchorLaneId") or action.get("laneId") or "",',
            '        summary=action.get("summary") or "",',
            '    )',
            'elif kind == "appendChild":',
            '    path = append_local_work_child_event(',
            '        root,',
            '        title=action.get("title") or "child milestone",',
            '        child_trajectory_id=action.get("childTrajectoryId") or "",',
            '        parent_event_id=action.get("parentEventId") or action.get("targetEventId") or action.get("currentEventId") or "",',
            '        kind=action.get("eventKind") or "task",',
            '        summary=action.get("summary") or "",',
            '        lane_id=action.get("laneId") or "",',
            '        child_lane_label=action.get("childLaneLabel") or action.get("laneLabel") or action.get("title") or "compound work",',
            '    )',
            'elif kind == "advanceChild":',
            '    path = advance_local_work_child_event(',
            '        root,',
            '        child_trajectory_id=action.get("childTrajectoryId") or "",',
            '        parent_event_id=action.get("parentEventId") or action.get("targetEventId") or "",',
            '        current_event_id=action.get("currentEventId") or None,',
            '    )',
            'elif kind == "closeChild":',
            ...
            '    if action.get("sourceEndpointTrajectoryId") or action.get("sourceEndpointEventId"):',
            '        source_endpoint = {',
            '            "trajectory_id": action.get("sourceEndpointTrajectoryId") or "",',
            '            "event_id": action.get("sourceEndpointEventId") or "",',
            '            "parent_event_id": action.get("sourceEndpointParentEventId") or "",',
            '            "compound_path": action.get("sourceEndpointCompoundPath") or "",',
            '        }',
            '    if action.get("targetEndpointTrajectoryId") or action.get("targetEndpointEventId"):',
            '        target_endpoint = {',
            '            "trajectory_id": action.get("targetEndpointTrajectoryId") or "",',
            '            "event_id": action.get("targetEndpointEventId") or "",',
            '            "parent_event_id": action.get("targetEndpointParentEventId") or "",',
            '            "compound_path": action.get("targetEndpointCompoundPath") or "",',
            '        }',
