...
Local Work Trajectory note: for task-like implementation, validation, review, or write-back work, proactively maintain Local Work Trajectory as agent-owned state. Direct `localTrajectory` mutation is leader/main/supervisor authority; bounded workers/subagents must not call it directly and must put progress/status suggestions in `Subagent Report.trajectory_update` for leader consumption. Worker report procedure: `docs/worker-trajectory-update-reporting.md`. Before the first leader-owned trajectory mutation, decide whether the work has distinct context streams; use `addLane` early for server/client/API contract/testing or similarly separable contexts, and keep one lane only for truly linear work. Use `localTrajectory start` when beginning a tracked task with no active trajectory; include `sourceGraphId` and `sourceNodeId` on `start` when a visible owning progress-map node is known so the trajectory is attached from birth. Use `append` for meaningful milestones, `advance` when the active milestone is complete, `merge` only for explicit fan-in, and `relate` only for visible relation metadata. Use `localTrajectory setAnchor` only when the current trajectory should move after start; pass both `sourceGraphId` and `sourceNodeId`, or pass neither to clear the anchor. Prefer visible current-phase, active-slice, or planning-gate nodes over leaving task work unanchored. If `localTrajectory` is expected but missing, report the MCP/tool exposure problem explicitly and do not ask the user to manually maintain trajectory nodes. Do not record MCP path or host-environment configuration checks into Local Work Trajectory unless the user explicitly asks to track that environment task.
