...
- **禁止终止对话**：每条回复必须以推进式提问结尾——即带有 AI 自身分析/推荐/判断的提问，而不是单纯列选项等用户选。"你是否希望 X？""请选择方向" 都算终止。正确做法是 AI 先给出自己的分析和倾向判断，再以 askQuestions 工具向用户确认。Phase 完成后尤其如此——应主动准备下一步分析文档并提问，不得停下等待。
