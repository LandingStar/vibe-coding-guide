...
2026-06-20 继续新增：`design_docs/stages/planning-gate/2026-06-20-background-scheduler-daemon-lifecycle-protocol.md` 已完成。当前 scheduler daemon 已有本地 lifecycle control contract 与 JSON control store：支持 start、heartbeat、pause、resume、cancel、shutdown、stale inspect，以及 `run_scheduler_daemon_lifecycle_once()` 对既有 bounded `run_scheduler_daemon_loop()` 的 lifecycle gate；stale heartbeat detection 同时兼容 epoch-second 字符串和 ISO-8601 timestamp。该 helper 只在 lifecycle state 为 `running` 时运行 bounded loop；paused/cancelled/stopped/stale 均不突变 scheduler state，cancelling / requested cancel 在 provider execution 前被消费为 cancelled。review evidence 位于 `review/background-scheduler-daemon-lifecycle-protocol-2026-06-20.md`；validation 结果为 lifecycle/daemon-loop focused pytest `16 passed`、full runtime orchestration pytest `191 passed`。该切片不启动 background daemon process、不添加 sleep/poll/watch/OS service、不运行 real provider、不绑定 UI、不实现 real sandbox provider、不突变 ExchangeArtifact/admission ledger、不从 scheduler code 突变 agent-owned Local Work Trajectory。

2026-06-20 继续新增：`design_docs/scheduler-daemon-lifecycle-cli-mcp-surface-direction-analysis.md` 已完成。该方向分析将 lifecycle control 的下一步收窄为 interaction/operator surface，而不是后台服务本身：推荐下一条窄 planning-gate 为 `Scheduler Daemon Lifecycle CLI/MCP Read-Write Surface`，先补 `doc-based-coding scheduler lifecycle <action>` 与 MCP `schedulerLifecycleControl` / `schedulerLifecycleRunOnce`，保持 fake-runtime run-once、显式路径、authority_split 和不自动刷新 projection；真实 background daemon host、watch/poll/process supervision、真实 Qoder provider、Host UX 绑定、ExchangeArtifact/admission ledger mutation 以及 scheduler code 对 agent-owned Local Work Trajectory 的突变继续推迟。

2026-06-20 继续新增：`design_docs/stages/planning-gate/2026-06-20-scheduler-daemon-lifecycle-cli-mcp-surface.md` 已完成。当前 lifecycle control 已有可供 Codex/MCP 和本地 operator 使用的 read-write surface：`doc-based-coding scheduler lifecycle inspect/start/heartbeat/pause/resume/cancel/shutdown/run-once` 通过 CLI 暴露，`schedulerLifecycleControl` / `schedulerLifecycleRunOnce` 通过 MCP 暴露，并保留显式路径、fake-runtime-only run-once、authority split 和 projection refresh 分离。该切片不启动 persistent background daemon、不添加 watch/poll/process supervision、不运行 real Qoder provider、不绑定 Host UX、不突变 ExchangeArtifact/admission ledger、不从 scheduler code/CLI/MCP 突变 agent-owned Local Work Trajectory。review evidence 位于 `review/scheduler-daemon-lifecycle-cli-mcp-surface-2026-06-20.md`；validation 结果为 py_compile passed、focused CLI `2 passed`、focused tracked MCP admission `2 passed`、scheduler MCP prompt `1 passed`、wider CLI `34 passed`、tracked MCP admission `5 passed`、runtime orchestration `191 passed`、doc-loop prompts `20 passed`。下一步推荐在 `Lifecycle Host UX Readback / Control Binding` 与 `Edit Lease Conflict Policy Expansion` 之间按产品优先级选择。

2026-06-20 继续新增：`design_docs/edit-lease-conflict-policy-expansion-direction-analysis.md` 已完成。该方向分析选择先推进 edit lease policy 的后端收窄，而不是立刻做 Host UX lifecycle binding：当前 `EditScopeLease`、subgraph child `allowed_artifacts` 与 write-back `contract.allowed_artifacts` 已经分别存在，但 scheduler admission 缺少可复用的结构化 conflict decision。推荐下一条窄 planning-gate 为 `Edit Lease Conflict Classifier And Admission Evidence`，先做纯 classifier / decision dataclass、路径规范化、冲突分类与 scheduler admission evidence；write-back unification、lease acquisition/expiry lifecycle、sandbox mount binding、Host UX/MCP readback 和 real sandbox provider 继续后置。

2026-06-20 继续新增：`design_docs/stages/planning-gate/2026-06-20-edit-lease-conflict-classifier-and-admission-evidence.md` 已完成。当前 scheduler admission 已有结构化 edit lease conflict evidence：`EditLeaseConflictDecision` 与 `classify_edit_lease_conflict()` 作为 scheduler-owned pure classifier 暴露，并通过 `AdmissionDecision.edit_lease_conflict` 接入 admission。该 classifier 现在覆盖 unsafe project-relative path、unsupported conflict policy、exact overlap、directory containment、directory overlap、denied artifact hit、read/write compatibility 与 `review-zone` overlap；其中 `review-zone` overlap 不自动运行，而是进入既有 `review_required` task state 并记录 `task_review_required` 事件。review evidence 位于 `review/edit-lease-conflict-classifier-and-admission-evidence-2026-06-20.md`；validation 结果为 py_compile passed、focused runtime classifier/admission tests `9 passed`、full runtime orchestration `198 passed`。下一步推荐继续后端收窄到 `Write-Back Enforcement Unification`，把 write-back planning 与 scheduler edit lease path/classification semantics 对齐。

2026-06-20 继续新增：`design_docs/stages/planning-gate/2026-06-20-write-back-enforcement-unification.md` 已完成。当前 write-back planning 已与 scheduler edit lease evidence shape 对齐：`WritebackEngine` 可消费 optional `edit_lease_conflict` / `edit_lease_decision` evidence，兼容 dict-like 与 dataclass-like `EditLeaseConflictDecision`；report payload 与 grouped child payload 会在本地 path / allowed-artifact 校验之后按 lease state 规划 disposition。`review_required` evidence 被记录为 `review_routed` skipped payload 且不执行写入，`blocked` / `waiting` evidence 被记录为 `blocked` skipped payload 且不执行写入，compatible / absent evidence 保持既有规划路径。review evidence 位于 `review/write-back-enforcement-unification-2026-06-20.md`；validation 结果为 py_compile passed、focused write-back lease evidence tests `5 passed`、full runtime orchestration `198 passed`。该切片不新增 lease lifecycle、不读取 live scheduler state、不改变 scheduler admission、不执行 review-routed payload、不绑定 Host UX/MCP readback、不改变 ExchangeArtifact admission、不从 scheduler/write-back code 突变 agent-owned Local Work Trajectory。

2026-06-20 继续新增：`design_docs/edit-lease-lifecycle-after-writeback-unification-direction-analysis.md` 已完成。该方向分析把 write-back enforcement unification 之后的后续路线收束为五个候选：edit lease acquisition/expiration lifecycle、sandbox mount binding over acquired leases、Host UX/MCP lease readback、scheduler daemon lifecycle Host UX binding、real sandbox provider spike。当前推荐下一条窄 planning-gate 为 `Edit Lease Acquisition And Expiration Lifecycle`，理由是 classifier 与 write-back evidence 已接通，但缺少 scheduler-owned lease requested/acquired/released/expired/revoked authority record；sandbox mount binding、Host readback 与 real provider 都应消费该 lifecycle record，而不是直接使用静态 `EditScopeLease` declaration。

2026-06-20 继续新增：`design_docs/stages/planning-gate/2026-06-20-edit-lease-acquisition-and-expiration-lifecycle.md` 已创建并激活。当前 active gate 将下一步实现限制在 scheduler-owned edit lease lifecycle：定义 lifecycle record/state/event，接入 acquire/release/expire/revoke helper，用显式 `now`/`timestamp` 处理 expiry，并通过 scheduler event replay 与 state snapshot round-trip 保留 lifecycle evidence。该 gate 明确不进入真实 sandbox enforcement、sandbox mount binding、Host UX/MCP readback、write-back live scheduler query、ExchangeArtifact 语义变化、Local Work Trajectory mutation 或 daemon expiry sweeping。

2026-06-21 继续新增：`design_docs/stages/planning-gate/2026-06-20-edit-lease-acquisition-and-expiration-lifecycle.md` 已完成。当前 scheduler edit lease lifecycle 已成为 scheduler-owned evidence：`SchedulerState.edit_lease_lifecycle` 保存 `EditLeaseLifecycleRecord`，task submission/admission/terminal transition/explicit expiry 分别写入 `requested`、`acquired|waiting|review_required|blocked`、`released|revoked`、`expired`；expiry 仅通过显式 `now` 输入触发。`SchedulerEvent` 可携带 `lease_id` 与 lifecycle snapshot，replay 与 snapshot round-trip 均保留 lifecycle evidence。review evidence 位于 `review/edit-lease-acquisition-and-expiration-lifecycle-2026-06-21.md`；validation 为 py_compile passed、focused lifecycle/classifier suite `17 passed`、full runtime orchestration `207 passed`。下一步推荐后端收窄到 `Sandbox Mount Binding Over Acquired Leases`，让 sandbox allocation 消费已 acquired lifecycle record。
