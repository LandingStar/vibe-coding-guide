...
| `trajectoryTeamContinuity` | `action*`, `callerRole`, `trajectoryId`, `laneId`, `leaderId`, `workerId`, `runtimeProvider`, `bindingId`, `ownershipId`, `replacementBindingId`, `newBindingId`, `sourceBindingId`, `noContinuityReason`, `taskId`, `deliveryId`, `timestamp`, `reason`, ledger/event-log paths, session/context refs, `workerReportRefs`, `auditRefs`, `includeInactive` | leader/main/supervisor/guide 侧 trajectory team continuity surface：通过共享 runtime dispatcher 执行 `inspect` / `resolve` / `assign` / `activate` / `suspend` / `resume` / `transfer` / `fork` / `release` / `noContinuity`，读写 trajectory-team roster、continuous worker binding、lane ownership 与 audit-only scheduler event evidence；拒绝 worker/subagent mutating caller roles并指向 `docs/worker-trajectory-update-reporting.md`；不运行 provider、不写 scheduler task state、不写 Local Work Trajectory | Trajectory team continuity / worker roster control |
...
- `trajectoryTeamContinuity` 不替代 `localTrajectory`：它维护 trajectory team roster、continuous worker binding 和 lane ownership evidence，不写 `.dbc/progress-graph/local-work-trajectory.json`。Local Work lanes 仍是规划/展示结构，team continuity 是实际 worker reuse 的审计和选择证据。
- `trajectoryTeamContinuity` 不替代 delivery supervisor：它不会运行 Codex/OpenCode/Qoder，也不会消费 delivery。binding reuse 与 delivery lease 审计仍由既有 delivery supervisor 路径记录；该 tool 只让 leader/main/supervisor/guide 显式 inspect/assign/resolve/transfer/release/fork/no-continuity。
- `trajectoryTeamContinuity` 采用 action enum 合并相关操作，而不是拆成多个 MCP tools；CLI `doc-based-coding scheduler trajectory-team <action>` 与 MCP `trajectoryTeamContinuity` 复用同一个 `run_trajectory_team_continuity_surface()` dispatcher，避免 CLI/MCP 权限和 readback 语义分叉。
...
- 不并入 `localTrajectory`：统一 workflow 只写 scheduler-owned snapshot/event-log、admission ledger、scheduler-loop evidence 与 scheduler-derived projection artifact；不会写 agent-owned `.dbc/progress-graph/local-work-trajectory.json`。
